...
async def show_collection(user, msg, page_num):
    ...
    if user.id in user_collections:
        ...
        if 0 <= page_num < len(collection):
            ...
            embed_to_show.set_footer(text=embed_to_show.footer.text.split(", ")[0] + ", " + embed_to_show.footer.text.split(", ")[1][0:5] + " --- " + f"{user_current_page[user.id] + 1}/{len(user_collections[user.id])}")
